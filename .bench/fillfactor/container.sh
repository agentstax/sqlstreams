...
# sqlstreams-bench-pg on :5433, postgres:18.4, 8 cpus / 8GB,
...
start)
  docker rm -f sqlstreams-bench-pg >/dev/null 2>&1 || true
  docker run -d --name sqlstreams-bench-pg \
    --cpus=8 --memory=8g \
    -p "$PGPORT":5432 \
  ...
  echo "sqlstreams-bench-pg ready on :$PGPORT"
  ;;
stop)
  docker rm -f sqlstreams-bench-pg >/dev/null
  echo "sqlstreams-bench-pg removed"
  ;;
*)
