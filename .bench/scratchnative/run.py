...
root=os.environ.get('VULKAN_NATIVE_ROOT') or pathlib.Path('/private/tmp/vulkan-native-session-path.txt').read_text().strip()
...
pg=os.environ.get('VULKAN_NATIVE_PG_BIN','/opt/homebrew/opt/postgresql@18/bin/')
...
(out/'topic-settings.json').write_text(sql('SELECT row_to_json(t) FROM vulkan.topic_config t WHERE name=\'orders\''))
if rca:
...
sql("UPDATE vulkan.worker_config SET target_instances=0,updated_at=now() WHERE name='exception_consumer'; CREATE EXTENSION pg_stat_statements;")
...
if maintenance_mode:
 ...
 for table in ['message_log_4_0','idempotency_key_4']:
  sql('ALTER TABLE vulkan.'+table+' SET (autovacuum_vacuum_insert_threshold=20000000,autovacuum_vacuum_insert_scale_factor=0,autovacuum_analyze_threshold=20000000,autovacuum_analyze_scale_factor=0)')
 (out/'maintenance-config.json').write_text(json.dumps(dict(mode=maintenance_mode,start_after_s=15 if maintenance_mode=='during' else duration,cost_delay_ms=maintenance_delay,cost_limit=200,wal_compression=maintenance_compression,thresholds=json.loads(sql("SELECT json_agg(s) FROM (SELECT relname,reloptions FROM pg_class WHERE relnamespace='vulkan'::regnamespace AND reloptions IS NOT NULL) s")))))
 maintenance_file=(out/'maintenance.log').open('w')
 maintenance_command=[pg+'psql',*base,'-d',name,'-v','ON_ERROR_STOP=1','-c',"SET vacuum_cost_delay='"+str(maintenance_delay)+"ms'",'-c','SET vacuum_cost_limit=200','-c',"SET wal_compression='"+maintenance_compression+"'",'-c','SHOW wal_compression','-c','VACUUM (ANALYZE, VERBOSE) vulkan.message_log_4_0']
...
(out/'worker-settings.json').write_text(sql('SELECT json_agg(w) FROM vulkan.worker_config w'))
...
try:
 if consumer_count:
  for i in range(20):
   ...
   active=sql("SELECT count(*) FROM vulkan.worker_instance i JOIN vulkan.worker_config w ON w.id=i.worker_id WHERE w.name='message_consumer'").strip()
  ...
  sql("UPDATE vulkan.worker_config SET target_instances=0,updated_at=now() WHERE name='exception_consumer'")
  ...
  time.sleep(2)
 exceptions=sql("SELECT count(*) FROM vulkan.worker_instance i JOIN vulkan.worker_config w ON w.id=i.worker_id WHERE w.name='exception_consumer'").strip()
 assert int(exceptions)==0,exceptions
 if not consumer_count:
  assert int(sql('SELECT count(*) FROM vulkan.worker_instance').strip())==0, 'worker process active'
 sql('CHECKPOINT')
 ...
 watch_sql="SELECT json_build_object('at',clock_timestamp(),'cursor',(SELECT row_to_json(c) FROM (SELECT claimed,committed,settled_head,pending_head,pending_xmax::text FROM vulkan.consumer_group_cursor_4 LIMIT 1) c),'visible_head',(SELECT max(id) FROM vulkan.message_log_4),'snapshot_xmin',pg_snapshot_xmin(pg_current_snapshot())::text,'sessions',(SELECT json_agg(s) FROM (SELECT pid,datname,backend_type,application_name,state,wait_event_type,wait_event,EXTRACT(EPOCH FROM clock_timestamp()-xact_start) transaction_age_s,client_addr::text,backend_xid::text,backend_xmin::text,CASE WHEN wait_event_type='Lock' THEN pg_blocking_pids(pid) ELSE ARRAY[]::int[] END blocking_pids,left(query,300) query FROM pg_stat_activity WHERE pid<>pg_backend_pid() AND (datname=current_database() OR backend_xid IS NOT NULL)) s))::jsonb;"
 ...
 if os.environ.get('RAW_PGX')=='1':
  assert rca
  command+=['-raw-sql',str(out/'protected-insert.sql'),'-topic-id',str(json.loads((out/'topic-settings.json').read_text())['id'])]
 ...
 with (out/'monitor.jsonl').open('w') as monitor:
  ...
  while any(p.poll() is None for p in [producer]+extra_producers):
   ...
   if analyze_interval and time.monotonic()-started>=next_analyze:
    ...
    analyze_query='ANALYZE vulkan.idempotency_key_4 (created_at)' if os.environ.get('ANALYZE_IDEMPOTENCY_ONLY')=='1' else 'ANALYZE vulkan.message_log_4; ANALYZE vulkan.idempotency_key_4'
    sql(analyze_query)
   ...
   snapshot=sql("SELECT (SELECT max(id) FROM vulkan.message_log_4),(SELECT max(committed) FROM vulkan.consumer_group_cursor_4); SELECT wait_event_type,wait_event,count(*) FROM pg_stat_activity WHERE datname=current_database() AND state='active' GROUP BY 1,2")
   table_stats=sql("SELECT json_agg(s) FROM (SELECT relname,n_live_tup,n_dead_tup,n_tup_ins,n_tup_del,n_mod_since_analyze,n_ins_since_vacuum,last_vacuum,last_autovacuum,vacuum_count,autovacuum_count,last_analyze,last_autoanalyze,analyze_count,autoanalyze_count FROM pg_stat_user_tables WHERE schemaname='vulkan' AND (relname='idempotency_key_4' OR relname LIKE 'message_log_4%')) s")
   cpu=subprocess.check_output(['ps','-axo','pid,ppid,%cpu,rss,time,comm'],text=True)
   relevant='\n'.join(line for line in cpu.splitlines() if 'postgres' in line or 'vulkan-native-scratch' in line)
   ...
   if retention_enabled:
    row['retention']=json.loads(sql("SELECT json_build_object('database_bytes',pg_database_size(current_database()),'key_bytes',pg_total_relation_size('vulkan.idempotency_key_4'),'partitions',(SELECT json_agg(c.relname ORDER BY c.relname) FROM pg_inherits i JOIN pg_class c ON c.oid=i.inhrelid WHERE i.inhparent='vulkan.message_log_4'::regclass))"))
   ...
   if backend_io:
    ...
    row['relation_io']=json.loads(sql("SELECT json_agg(s) FROM pg_statio_user_tables s WHERE schemaname='vulkan'"))
    row['index_io']=json.loads(sql("SELECT json_agg(s) FROM pg_statio_user_indexes s WHERE schemaname='vulkan'"))
 ...
 (out/'cleanup-profile.json').write_text(sql("SELECT json_agg(s) FROM (SELECT calls,total_exec_time,rows,shared_blks_read,shared_blks_hit,query FROM pg_stat_statements WHERE query LIKE 'DELETE FROM vulkan.idempotency_key_4%') s"))
 (out/'actual-batches.txt').write_text(sql("SELECT count(*) transactions,min(messages),round(avg(messages),2),max(messages) FROM (SELECT xmin,count(*) messages FROM vulkan.message_log_4 GROUP BY xmin) batches"))
 (out/'verification.txt').write_text(sql("SELECT count(*) FROM vulkan.message_log_4; SELECT count(*) FROM vulkan.worker_instance i JOIN vulkan.worker_config w ON w.id=i.worker_id WHERE w.name='exception_consumer'; SELECT name,target_instances FROM vulkan.worker_config WHERE name='exception_consumer'"))
 if rca and os.environ.get('VERIFY_SEMANTICS')=='1':
  (out/'semantics.json').write_text(sql("SELECT json_build_object('messages',(SELECT count(*) FROM vulkan.message_log_4),'keys',(SELECT count(*) FROM vulkan.idempotency_key_4),'nondefault_messages',(SELECT count(*) FROM vulkan.message_log_4 WHERE options IS NOT NULL OR routing_key IS NOT NULL OR message_key IS NOT NULL OR schema_version<>1),'non_v7_keys',(SELECT count(*) FROM vulkan.idempotency_key_4 WHERE substring(idempotency_key::text,15,1)<>'7'))"))
...
if not consumer_count and os.environ.get('VERIFY_IDENTITIES')=='1':
 identity_counts=json.loads(sql("SELECT json_build_object('rows',count(*),'distinct_sequences',count(DISTINCT (payload->>'sequence')::bigint),'minimum_sequence',min((payload->>'sequence')::bigint),'maximum_sequence',max((payload->>'sequence')::bigint)) FROM vulkan.message_log_4"))
...
if not consumer_count:
 assert int(sql('SELECT count(*) FROM vulkan.worker_instance').strip())==0, 'worker became active'
...
if os.environ.get('PLAN_PROBE')=='1':
 (out/'cleanup-catalog.json').write_text(sql("SELECT json_build_object('indexes',(SELECT json_agg(i) FROM pg_indexes i WHERE schemaname='vulkan' AND tablename='idempotency_key_4'),'statistics',(SELECT json_agg(s) FROM pg_stats s WHERE schemaname='vulkan' AND tablename='idempotency_key_4'),'table',(SELECT row_to_json(t) FROM pg_stat_user_tables t WHERE schemaname='vulkan' AND relname='idempotency_key_4'))"))
 for mode in ['force_generic_plan','force_custom_plan']:
  query="BEGIN; SET LOCAL plan_cache_mode='"+mode+"'; PREPARE cleanup_probe(timestamptz,bigint) AS DELETE FROM vulkan.idempotency_key_4 WHERE idempotency_key IN (SELECT idempotency_key FROM vulkan.idempotency_key_4 WHERE created_at < $1 LIMIT $2); EXPLAIN (ANALYZE,BUFFERS,FORMAT JSON) EXECUTE cleanup_probe(now()-interval '24 hours',1000); ROLLBACK;"
