...
// invariant e2e test: the migrate engine's guarantees, exercised with a FIXTURE
...
const (
	stepwise = "invariant_stepwise" // built version-by-version via the fixture steps
	fresh    = "invariant_fresh"    // created in its final shape directly
	maxV     = 4
)
...
// one v1 baseline row -- the e2e test only ever borrowed the system scope, and its
