...
func run() (err error) {
	...
	fmt.Println("\n✅ IDEMPOTENCY KEYS E2E TEST PASSED")
...
func duplicateKeyScenario(ctx context.Context, pool *pgxpool.Pool) {
	...
	for attempt := range 3 {
		...
		if attempt == 0 && (produced.Duplicate || produced.Id == 0) {
			common.Die(fmt.Sprintf("first call must not report Duplicate and must carry a stored id, got Duplicate=%t Id=%d", produced.Duplicate, produced.Id))
		}
		if attempt > 0 && (!produced.Duplicate || produced.Id != 0) {
			common.Die(fmt.Sprintf("retried call under the same key must report Duplicate=true Id=0, got Duplicate=%t Id=%d", produced.Duplicate, produced.Id))
		}
	}

	assertMessageLogCount(ctx, ds, tp.Id, 1)
	if calls != 3 {
		common.Die(fmt.Sprintf("producerFunc ran %d times, want 3 -- re-running it is the documented contract", calls))
	}
...
func sweepScenario(ctx context.Context, pool *pgxpool.Pool) {
	...
	common.Must(janitorDatastore.SweepExpiredIdempotencyKeys(ctx, tp.Id, ttl, batchSize))
...
func configRoundTripScenario(ctx context.Context, pool *pgxpool.Pool) {
	...
	if tp1.IdempotencyKeyTTL != 24*time.Hour {
		common.Die(fmt.Sprintf("default IdempotencyKeyTTL = %v, want 24h", tp1.IdempotencyKeyTTL))
	}
	...
	if tp2.IdempotencyKeyTTL != 24*time.Hour {
		common.Die(fmt.Sprintf("re-registered IdempotencyKeyTTL = %v, want 24h", tp2.IdempotencyKeyTTL))
	}
	...
	if tp3.IdempotencyKeyTTL != 2*time.Hour {
		common.Die(fmt.Sprintf("re-declared IdempotencyKeyTTL = %v, want 2h", tp3.IdempotencyKeyTTL))
	}
	...
	if !errors.Is(err, stream.ErrStreamConfigMismatch) {
		common.Die(fmt.Sprintf("expected ErrStreamConfigMismatch for a changed PartitionSize, got %v", err))
	}
...
func assertIdempotencyKeysCount(ctx context.Context, ds *iDatastore.PostgresDatastore, streamId int64, want int) {
	...
	if count != want {
		common.Die(fmt.Sprintf("%s.%s has %d rows, want %d", ds.Schema, stream.IdempotencyKeyTable(streamId), count, want))
	}
}
