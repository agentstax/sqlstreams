...
// its handler prints "handler blocked" and ignores its context forever. The
// message timeout is five minutes, so an exit long before that is one that
// did not wait it out.
...
func run() error {
	...
	cfg := &sqlstreams.ConsumerConfig{Message: &sqlstreams.MessageOptions{Timeout: 5 * time.Minute}}
	consumer, err := client.Stream[common.Work](*streamName).Consumer("signal.e2e").Register(ctx, cfg)
	if err != nil {
		return err
	}
