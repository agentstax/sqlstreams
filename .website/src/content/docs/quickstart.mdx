---
title: Quickstart
description: Set up a Go example, produce a message, and consume it from your Postgres database.
---

import ThreadAside from '../../components/thread-aside/thread-aside.svelte';

Produce and consume one welcome-email message using two Go programs and
an existing Postgres database. The consumer prints the request; it does
not send an email.

## 1. Prepare the example

You need Go 1.27+ and a local Postgres database. These examples use
`app_db` on `localhost:5432`, with role `app_user`. The role must be able
to create schemas and tables in that database. Substitute your own
connection details in both programs if they differ.

Create a directory and install the library:

```sh
mkdir vulkan-quickstart
cd vulkan-quickstart
go mod init example.com/vulkan-quickstart
go get github.com/agentstax/vulkan
mkdir -p cmd/produce cmd/consume
export PGPASSWORD='your-local-database-password'
```

Run the commands below from this directory, in the same terminal so both
programs receive `PGPASSWORD`. The first topic registration creates
Vulkan's shared tables and the topic's tables in the `vulkan` schema.
There is no separate migration command for this example.

## 2. Produce a message

Save this as `cmd/produce/main.go`:

```go
package main

import (
	"fmt"
	"os"

	vulkan "github.com/agentstax/vulkan/pkg/vulkan"
)

type WelcomeEmail struct {
	UserId string `json:"user_id"`
}

func (WelcomeEmail) SchemaVersion() int { return 1 }

func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
}

func run() error {
	ctx, stop := vulkan.LifecycleContext(nil)
	defer stop()

	pool, err := vulkan.NewPostgresPool(ctx, "app_user", os.Getenv("PGPASSWORD"), "localhost", "app_db", nil)
	if err != nil {
		return err
	}
	defer pool.Close()

	client, err := vulkan.NewClient(ctx, pool, nil)
	if err != nil {
		return err
	}

	emails := client.Topic[WelcomeEmail]("signup.welcome-email")
	if _, err := emails.Register(ctx, nil); err != nil {
		return err
	}

	producer, err := emails.Producer().Register(ctx, nil)
	if err != nil {
		return err
	}

	produced, err := producer.Produce(ctx, &WelcomeEmail{UserId: "user-123"}, nil)
	if err != nil {
		return err
	}
	fmt.Printf("produced message id=%d\n", produced.Id)
	return nil
}
```

Run it:

```sh
go run ./cmd/produce
```

The output includes `produced message id=1` on a fresh topic; the id may
be higher if the topic already contains messages. Each run produces
another message.

`WelcomeEmail` is stored as JSONB. Its `SchemaVersion` method identifies
the payload version, which the consumer type must also declare.
[Schema Versions](/guides/schema-versions/) explains how versions share a topic.

## 3. Consume the message

Save this as `cmd/consume/main.go`:

```go
package main

import (
	"context"
	"errors"
	"fmt"
	"os"

	vulkan "github.com/agentstax/vulkan/pkg/vulkan"
)

type WelcomeEmail struct {
	UserId string `json:"user_id"`
}

func (WelcomeEmail) SchemaVersion() int { return 1 }

func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
}

func run() error {
	ctx, stop := vulkan.LifecycleContext(nil)
	defer stop()

	pool, err := vulkan.NewPostgresPool(ctx, "app_user", os.Getenv("PGPASSWORD"), "localhost", "app_db", nil)
	if err != nil {
		return err
	}
	defer pool.Close()

	client, err := vulkan.NewClient(ctx, pool, nil)
	if err != nil {
		return err
	}

	emails := client.Topic[WelcomeEmail]("signup.welcome-email")
	registered, err := emails.Get(ctx)
	if err != nil {
		return err
	}
	if registered == nil {
		return errors.New("topic not registered -- run the producer first")
	}

	sender := emails.Consumer("email-sender")
	consumer, err := sender.Register(ctx, nil)
	if err != nil {
		return err
	}

	return consumer.Consume(ctx, receiveEmail, nil)
}

func receiveEmail(ctx context.Context, email *WelcomeEmail) error {
	fmt.Printf("received welcome email request for %s\n", email.UserId)
	return nil
}
```

Run it:

```sh
go run ./cmd/consume
```

The output includes:

```text
received welcome email request for user-123
```

Startup also writes logs to stderr. On a fresh installation,
`alert evidence is insufficient` means an alert check has too little
metric history to evaluate yet; it does not mean the message failed.
[Alert history](/concepts/alert-history/#recording-and-evidence-gaps)
explains missing evidence.

The process keeps waiting for messages. Press Ctrl-C to stop it:
`LifecycleContext` turns SIGINT and SIGTERM into cancellation, and
`Consume` drains its in-flight handlers before returning.

The new `email-sender` group reads retained history by default, including
the message produced before it started. Restarting the same group resumes
its existing cursor. If a previous run already consumed the message,
stop the consumer, run the producer again, then restart the consumer.

Returning `nil` succeeds; an ordinary error schedules a retry.
[Handler Outcomes](/concepts/handler-outcomes/) covers terminal errors and
requested delays. A real email handler must tolerate duplicate delivery
and pass its context to blocking calls: crashes and timeouts can cause
at-least-once redelivery. See [Consumer Timeouts](/guides/consumer-timeouts/)
and [Side Effects & Retries](/guides/side-effects-and-retries/).

<ThreadAside label="NOTE" title="Keep upkeep running">
`Consume` runs the system manager beside the session by default. It
advances committed cursors and carries the deployment's maintenance.
A deployment that only produces must also run a
[standalone manager](/reference/manager/).

</ThreadAside>

## 4. Continue with your application

- [Transactional Produce](/guides/transactional-produce/) puts a business
  write and its message in one commit.
- [Message Lifecycle](/concepts/lifecycle/) explains retries, leases, and
  the rows you can inspect when a handler fails.
- [Fan-out, Retention & Replay](/concepts/fan-out/) explains independent
  groups and how long their source messages remain available.
- [Consumer](/reference/consumer/) lists configuration fields, defaults,
  and session options.
