...
| `Status`, `Messages` | `sqlstreams schedule get` / `get --messages` | per-group outcomes, read from the target stream's `delivery_log` by message key; a `succeeded` count needs the target stream's `DeliveryLogMode: all` (the default keeps failures only) - `Register` warns [SS0058](/errors/SS0058) when it isn't |
