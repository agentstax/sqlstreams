...
`Producer().Register` resolves the topic and `Produce` writes. For a
...
`invoices` is topic id `4`. `Scheduler("invoices.nightly").Register[InvoiceRun]` writes:
...
| `Status`, `Messages` | `vulkan schedule get` / `get --messages` | per-group outcomes, read from the target topic's `delivery_log` by message key; a `succeeded` count needs the target topic's `DeliveryLogMode: all` (the default keeps failures only) - `Register` warns [VK0058](/errors/VK0058) when it isn't |
