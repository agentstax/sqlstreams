...
A topic retains messages independently of consumption. Each consumer
...
They share database capacity and the topic's retention policy.

## Independent groups

On `orders.created`, `email-receipts` sends receipts while
`fraud-screening` scores the same orders. Each group registers against
the same topic handle:

```go
orders := client.Topic[OrderCreated]("orders.created")
...
A group that only needs new traffic declares `Start: vulkan.Head()`.
...
Suppose `orders.created` (topic id `1`) has a visible head of `1204318`.
...
For topic id `1`, inspect the cursors:
...
SELECT
    ...
    c.claimed,
    c.committed,
    (SELECT COALESCE(max(id), 0) FROM vulkan.message_log_1) - c.committed AS cursor_distance
FROM vulkan.consumer_group_cursor_1 c
JOIN vulkan.consumer_group_config g ON g.id = c.consumer_group_id;
```
...
`TopicConfig.RetentionTTL` defaults to `0`, which retains messages
...
is deleted too. See [Topic](/reference/topic/#config) for the fields.
