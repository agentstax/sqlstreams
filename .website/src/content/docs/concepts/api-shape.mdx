...
`github.com/agentstax/sqlstreams/pkg/sqlstreams`; the per-handle lookup tables are
...
([SS0061](/errors/SS0061/) for a stream, [SS0059](/errors/SS0059/) for a
group, [SS0062](/errors/SS0062/) for a schedule), and an identical one
...
tell is the same SS0059 line on every restart of either one. One declaring
...
`sqlstreamstest.NewClient(t)` stands the real library up in a schema it drops
at cleanup, so a handler test runs against Postgres and nothing fake. Not
built; a reference page lands with the package.
