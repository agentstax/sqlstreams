...
| verb | returns | notes |
| --- | --- | --- |
| `Register(ctx, cfg)` | `*ProducerInstance[T]` | resolves the stream, `ErrStreamNotFound`; `nil` cfg is the defaults; logs [SS0063](/errors/SS0063/) when the stream's upkeep workers have no live instance |
...
| `SlowProduceThreshold` | `0` (off) | a produce running longer logs [SS0038](/errors/SS0038/); the closure verbs include your closure |
...
- A deployment that only produces runs no upkeep. Its `Register` logs
  [SS0063](/errors/SS0063/) naming the unclaimed rows; run
