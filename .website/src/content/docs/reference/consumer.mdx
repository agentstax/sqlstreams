...
| verb | returns | notes |
| --- | --- | --- |
| `Register(ctx, cfg)` | `*ConsumerInstance[T]` | resolves the stream, writes the declaration, creates the cursor row on first registration; `nil` cfg is the defaults; newest declaration wins, and a differing one logs [SS0059](/errors/SS0059/) |
...
| `SlowDispatchThreshold` | `0` (off) | a dispatch running longer logs [SS0039](/errors/SS0039/) |
...
consumer workers start and on config refresh. [SS0105](/errors/SS0105/) warns when a queued
...
- A group's config wants exactly one declaring service. Two services
  ...
  restart, and the same [SS0059](/errors/SS0059/) line on every restart is
  the tell.
