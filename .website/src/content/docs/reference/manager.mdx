...
| --- | --- |
| `0` | nothing, everywhere; the loop logs [SS0035](/errors/SS0035/) and reconciles no rows |
...
- `ClientConfig.DisableManager` is the per-process opt-out: consumer pods
  beside a dedicated `sqlstreams manager run`, and consumers under a database
  role with no DDL rights, since the stream janitor creates and drops
  partitions.
- Admin verbs run nothing and the producer runs nothing. A deployment that
  ...
  [SS0063](/errors/SS0063/) naming the unclaimed rows, and the one-minute
