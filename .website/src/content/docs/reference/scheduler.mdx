...
failure. `Register` declares it against a target topic, a cron expression,
...
| verb | returns | notes |
| --- | --- | --- |
| `Register(ctx, topicName, cron, payload, cfg)` | `*SchedulerInstance[T]` | newest declaration wins, and a differing one logs [VK0062](/errors/VK0062/); `nil` cfg is the defaults; `ErrTopicNotFound` |
...
| `Metadata` | `nil` (stored as `{}`) | opaque JSON on the row, shown by `vulkan schedule get`; not part of the produced message |
...
- The schedule name lives on the handle, so it cannot trade places with
  the target topic in `Register`. Every verb but `Register` is keyed by the
  name alone.
