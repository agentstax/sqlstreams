...
| verb | returns | notes |
| --- | --- | --- |
| `Register(ctx, streamName, cron, payload, cfg)` | `*SchedulerInstance[T]` | newest declaration wins, and a differing one logs [SS0062](/errors/SS0062/); `nil` cfg is the defaults; `ErrStreamNotFound` |
...
| `Metadata` | `nil` (stored as `{}`) | opaque JSON on the row, shown by `sqlstreams schedule get`; not part of the produced message |
