...
description: DiagnosticError and DiagnosticEvent — the accessors on every SQL-coded error and log event, errors.Is against the Err variables, and Owner.
---

Every error SQLStreams returns with a `SQL` code is a `*sqlstreams.DiagnosticError`,
...
- Every code has its thread on [Troubleshooting](/boards/troubleshooting/),
  and `sqlstreams explain SQL0005` prints the same page offline.
