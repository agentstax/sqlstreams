...
description: DiagnosticError and DiagnosticEvent — the accessors on every SS-coded error and log event, errors.Is against the Err variables, and Owner.
---

Every error SQLStreams returns with a `SS` code is a `*sqlstreams.DiagnosticError`,
...
| --- | --- | --- |
| `GetCode()` | `string` | the `SS` code, the link to its page |
...
- `Recovery` is read-only. A service can attach context to
  `ErrStreamNotFound` and still match it with `errors.Is`, and it cannot
...
- Every code has its thread on [Troubleshooting](/boards/troubleshooting/),
  and `sqlstreams explain SS0005` prints the same page offline.
