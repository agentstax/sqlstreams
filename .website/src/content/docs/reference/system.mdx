...
| verb | returns | notes |
| --- | --- | --- |
| `Register(ctx, cfg)` | `error` | creates the schema and the control-plane tables when missing ([SS0064](/errors/SS0064/) when the role cannot); `nil` cfg is the defaults; a differing redeclaration replaces the stored worker rows |
