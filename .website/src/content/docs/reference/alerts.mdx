...
`metrics_collector_progress`. Each has a SS code, so
`sqlstreams explain SS0094` and its page say what an active one means. The
...
- The three stream alerts require two minutes of consecutive unhealthy
  ...
  stream count. See [measurement history](/concepts/alert-history/) for the rules.
