...
[SS0064](/errors/SS0064/), and the fix is to create the schema yourself and
...
- `DisableManager` is for consumer pods beside a dedicated
  `sqlstreams manager run`, and for consumers under a database role with no DDL
  rights, since the stream janitor creates and drops partitions.
