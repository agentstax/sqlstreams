...
- Longer candidate validation completed (`scratch_233728`, 600s): same 10k cleanup batches / 30s timeout / 1s poll, both TTLs2m, unpaced four callers x250, pools8/8, GOMAXPROCS4 each, durable native PG18 baseline. Identity validation ceiling alone raised40m to200m in isolated scratch build to prevent early completion; code and build hash archived. Runner captures exact final expired count before database removal; 100GB total-footprint and40GiB free-space guards retained. Earlier startup `scratch_233611` rejected200m against the old binary ceiling before load; database removed, not a throughput sample.
- Ten-minute candidate verdict (`janitor_ctid_600_233728` / `scratch_233728`): 37,679,750 produced/consumed exactly once; zero application errors or duplicate identities, consumer p99 3370ms. Last five minutes producer/consumer 52,576/52,608msg/s (last60s65,288/65,219; whole-run producer62,799 includes opening burst). Five logged janitor failures: all idempotency sweeps, three also partition sweeps; 30s deadlines caused retry backoff1/2/4/8/16s. No swapouts, eight completed checkpoints. Sampled candidate plan retained timestamp Index Scan + Tid Scan, so no observed return to the full-table target scan. 29,024,500 committed key deletions in131.631s SQL (~4.54us/key),4.791GBcleanupWAL. Exact final retained8,655,250 keys, expired1,694,000 at producer-stop cutoff. Estimated expired-backlog per-minute peaks after minute3 were5.59/5.23/5.00/4.88/4.78/3.47/3.86m: recovery rather than continuously rising backlog, but estimates use delayed deletion stats and include aborted work; final direct count is authoritative. Database size in minutes7–10 cycled~9.6–14.5GB; key allocation still rose2.06→2.51GB across minutes5–10, so indefinite physical stability is unproven. Native peak34.13GB, minimumhostfree89.19GB,100GBguard retained. Disposable DB removed; native baseline restored, retainednative8.63GB. Candidate stays scratch-only and capacity-ineligible because maintenance failed. Next isolated configuration test: same10k batches andquery, increase only cleanup timeout30→60s to test whether deadline/backoff is preventing smooth cleanup; no evidence yet requiring key-table partitioning. Evidence includes exact source/hash/settings, final count, trend analyzer, query plan, counters and verdict.
- Offline timeout review of `scratch_233728` (supersedes the proposed blanket30→60s test): `CleanupTimeout` is separately applied to serial stages: partition drops → partial message-row sweeps → idempotency sweeps → other cleanup. Key batches commit independently; deadlines do not undo preceding batches. Five preceding30s windows show ~4.17/3.56/3.09/4.44/3.23m key deletions by delayed table counters (approximate; can include aborted work).1100 distinct key transaction IDs observed, longest sampled active age1.803s; not evidence of a single30s hung DELETE. Key sampled active time:28.1%data reads,7.0%data writes,19.9%WALWrite lock,5.2%WAL write I/O,36.6%CPU/runnable; no heavyweight Lock wait observed. Coverage541.83/600s after excluding gaps>.5s; maxgap2.838s. Completed message-row DELETEs consumed160.302sSQL for4,972,255 rows;144.009s/4,122,255 rows belonged to partitions later dropped whole during this run. Message sampled active time66.6%data reads+23.0%data writes. Worked case orders partition8:740k row deletes/19.330sSQL, message sweep cancelled19:41:20.058, following key stage made~4.17m deletion progress then cancelled19:41:50.068, partition8 dropped19:41:55.732. This points to costly partial message cleanup delaying key cleanup, while the same timeout increase would extend BOTH stages and confound attribution. Next recommended fence: scratch-only bypass of partial message-row sweeps, retaining partition drops/key cleanup and existing30s/10k settings; compare throughput/key backlog/failures. Explicit semantic tradeoff: messages can remain until whole-partition expiry; sparse/unfinished partitions prevent treating this as a default production fix. No new load/library edits in this review. Analyzer, per-timeout windows, bounded original PostgreSQL log, message query costs and conclusions saved in run evidence.
- Partial message-sweep isolation completed (`scratch_235841`,600s): frozen `scratch_233728` candidate source, only orders/stream4 call to `SweepExpiredPartitions` bypassed. Whole-partition drops, key cleanup, other stream maintenance, both2mTTLs,10k batches,30s cleanup timeout,1s poll,4x250 production,pools8/8,CPU/memory settings and native durable PG unchanged. Diagnostic explicitly relaxes row expiry until whole-partition drop; not a production default. Scratch build/fmt/vet and janitor package race tests passed before load.200midentity ceiling and100GB storage guard retained. Source/delta/hash archived, final SQL must confirm zero partial orders message DELETEs.
- Partial-sweep fence result (`janitor_no_row_sweep_235840` / `scratch_235841`):43,218,000 produced and consumed exactly once, zero application errors/duplicates, planned600s(actual601.033s including final producer work). Common300–600s producer/consumer59,125/59,078msg/s versus52,576/52,608 baseline:producer+12.46%. Last60s53,161/53,265, so do not cherry-pick the whole-run71,906/s opening-burst average. Consumerp991118ms versus3370; maxsampledhandlerbacklog37,808 versus335,000, declining overall trend. Key cleanup30s deadline failures5→0, final expired keys1,694,000→0 at producer-stop cutoff, retained6,891,750keys.36,326,250keysdeleted/152.559sSQL/5.870GBWAL. Live estimated expired backlog afterminute3 peaked~660k versus prior~5.59m; key allocation2.28GB atminute6 to2.45GB atfinish, largely flat betweenvacuumcycles. One DIFFERENT maintenance failure: DROP TABLE message_log_4_34 lock timeout20:08:23.503, warning20:08:24.340,1.039sretry, successful drop20:08:26.906; blocker not identified from samples, no claim it was autovacuum. No swapouts; nativepeak32.76GB/freehostmin89.71GB; guardwithin100GB. SQL verifies ZERO partial orders message DELETEs; all source files identical to baseline exceptinstance.go, commands/runtime settings match after normalizingbinary/database/profilepaths. Scratch fmt/build/vet/racetests passed. Sourcepatch, runtimechecks, finalcounts, trends, lock-event evidence andverdict archived. Database deleted, durable native PG restored. Supports partial message sweeps as material throughput/scheduling cost; key partitioning is not established as necessary. Whole-partition message retention now has direct supporting evidence, with sparse/current-partition behavior needing a production design. No library bypass applied; retained scratch-only, capacity-ineligible due recovered DROP lock timeout and single comparison.
- IMPORTANT external-load correction (user flagged concurrent integration tests; supersedes causal interpretation of the preceding comparison): saved host-process samples confirm test processes overlapped BOTH runs. Baseline `scratch_233728` production19:37:32–19:47:32EDT had repeated consume.test/worker.test observations19:37:39.912–19:45:40.062. Candidate `scratch_235841` production19:58:45–20:08:46EDT had conventions.test19:59:24.121 and consume.test+worker.test20:00:10.782–20:00:21.274. These are sampled presence bounds, not exact process lifetimes. OrbStack Helper accumulated91.00CPU-seconds in baseline versus32.63 in candidate, peaks325.2%/250%; this is aggregate VM activity, not test-specific CPU or disk accounting. Docker lifecycle history query23:30Z–00:12:30Z only returned container events from20:04:59EDT onward, insufficient to recover earlier test DB lifetimes. First retained postgres:18 start happy_bohr20:08:55.423 was AFTER producer and consumer completion, during verification/cleanup; subsequent testcontainers starts20:09:15.917/20:09:31.211/20:11:52.139/20:12:15.388 also post-load. Thus container history alone cannot clear the runs. Withdraw12.5% as a causal gain and treat comparative cleanup improvements as confounded; preserve observed counts, errors, SQLcosts andverifiedcode/configdelta. Mark both verdicts external-load-confounded. Before further tuning, repeat BOTH configurations with competing tests absent, capture container events from before setup, and flag/abort overlapping test activity. No user containers stopped or modified. Evidence: external-load-audit.json, container-events.jsonl and host-test-processes.json in run evidence.
- Quiet repeat pair started (`quiet_pair_20260910_002556`):600s rows_on followed by600s rows_off, existing frozen binaries, same30s cleanup/10k batch/1s poll and application configuration.20s quiet preflight; continuous Docker events captured from before setup;1Hz process guard rejects Go/test binaries or go-build executables. New/restarted/unpaused containers or non-health-check container exec create violation marker; runner aborts and cleans its scratch DB; second arm suppressed after a violation. Guard heartbeat required; source/scripts archived. Existing PostgreSQL17 Compose container left untouched and recorded.100GB storage guard preserved. This replaces the confounded earlier comparison.
- Quiet-pair attempt `quiet_pair_20260910_002556` interrupted: process guard detected three new `go` processes (PIDs63708/63709/63711, parents63705/98199) at20:28:05.641EDT during `scratch_002618`; runner aborted, owned DB removed and native durable baseline restored. Rows-off arm never started. No new testcontainer lifecycle events were captured before stop. Conservative guard records command names only, so Go subcommands and interference magnitude are unknown; excluded attempt is not performance evidence. Waiting for a coordinated quiet interval before repeating.
- Quiet guard refinement and retry (`quiet_pair_20260910_003051`): two Go-process parents from the aborted attempt resolved to gopls(PID98199), so classifying all Go commands as competing tests was overbroad. Guard now reads the Go subcommand, records env/version/list/help as metadata, and still rejects other Go actions/test executables or new containers. Commands that exit before classification are recorded as unclassified metadata, not evidence of a clean absence; continuous container capture remains authoritative for new test DBs. Repeating both600s arms with corrected guard, existing frozen binaries and identical runtime configuration.
- Corrected quiet-pair retry `quiet_pair_20260910_003051` also interrupted: actual `go vet` observed at20:31:47.367EDT(PID66345,parent66338,52% sampledCPU). The guard now distinguishes metadata commands, so this trigger was an actual competing verification command. First arm `quiet_rows_on_003112` aborted, benchmark DB deleted, durable native PG baseline restored; rows_off never started. Neither interrupted attempt is throughput evidence. Further load held pending a coordinated~25minute interval with external builds/tests paused; user asked asynchronously.
- User confirmed other sessions have stopped e2e/integration tests; relaxed repeat pair `relaxed_pair_20260910_003620` launched. Same600s rows_on then600s rows_off, frozen binaries and native durable settings. Go tooling/test executables and non-test Docker actions recorded only, not abort triggers. Newly created/started testcontainers or PostgreSQL containers still invalidate the run; event capture begins before setup, heartbeat/storage protections retained. This corrects prior overstrict go-vet aborts and keeps the experiment focused on partial message sweep cost.
- Relaxed repeat completed (`relaxed_pair_20260910_003620`; rows_on `scratch_003641`, rows_off `scratch_004731`): both600s, frozen binaries; normalized application commands and all recorded PostgreSQL settings identical. Continuous Docker capture contains only existing PG17 health checks, no new test/PG containers. Go tooling recorded only during baseline (including a vet sampled over57s and several short go test commands); no automatic tooling abort under agreed rules. This is not a fully idle-host randomized comparison; exact gain remains provisional, not invalidated merely by tooling presence.
- Matched300–600s producer/consumer rows_on56,097/56,060msg/s versus rows_off63,610/63,626 (+13.39% observed producer). Whole-run production42,334,500 versus40,733,500: baseline had the larger startup burst, so whole-run averages reverse the late-window result. Both consumed every produced identity once, zero application errors/duplicates. Consumerp991310→1479ms (no latency improvement); max estimated handlerbacklog161,808→67,698; committed-ID-distance first/last30s median86,233→39,212 baseline and61,771→59,928 bypass, no sustained consumer divergence demonstrated. Both have maintenance failures: baseline one partial-sweep30s deadline; bypass one partition-drop lock timeout/retry. Zero key-cleanup deadlines in BOTH, unlike old confounded baseline. Neither qualifies as a zero-maintenance-error maximum.
- Mechanism evidence: successful partial message deletes4,570,759rows/193.145sSQL→ZERO; key deletes33,956,750/160.948sSQL→32,887,750/122.639sSQL (~4.74→3.73us/key). Final directly counted expired keys2,166,000→0 at producer-stop cutoff, sampled after workers stop; retained8,377,750→7,845,750. Live estimated expired peaks afterminute3 up to4.896m baseline versus524k bypass (delayed stats are approximate). Key allocation minutes5–10 baseline2.276→2.936GB versus1.617→2.079GB, candidate flat final~2minutes; indefinite space stability unproven. No swapouts, but compression activity differed. Native peak33.72→28.67GB; minimum host free88.53→94.43GB,100GB benchmark cap held. Owned DBs removed and native durable baseline verified restored. Evidence includes comparison-verdict.json, archived analyzer/guard/drivers, exact identities, settings, SQL, key trends and process/container audit. No library changes. Recommendation: pursue avoiding redundant partial deletion while preserving a bounded cleanup path for sparse/current partitions; separately inspect the repeated DROP lock timeout using existing evidence before another load test. Key-table partitioning and blanket timeout increases remain unsupported as next fixes.
- Bounded partial-sweep grace implemented [0738]: per-stream worker metadata partial_sweep_grace_period, nonnegative duration in nanoseconds, default0. Existing oldest-row probe gates first batch at TTL+grace; subsequent batches drain to normal TTL cutoff. Drops/cursor guards/key cleanup unchanged. Start/debug logs expose configuration and deferral; no new metrics. Native PG race tests verify sparse fallback, multi-batch drain, cursor protection, zero grace and whole drops; touched builds/vet pass. Disposable test schemas cleaned by fixture. Next performance experiment should compare configured grace against zero with production code; 30s remains an example, not a validated choice. No new throughput run in this implementation turn.
- Bounded-grace benchmark started `grace_pair_20260910_011414`: frozen current working-tree library, same binary for both600s runs, orders partial_sweep_grace_period0→30s; both use current production ordered UUID key cleanup (not prior scratch CTID substitution). Scratch-only30s cleanup timeout retained in both assemblers,4x250 producer,pools8/8,2mTTLs,10kcleanup/1spoll,100GBcap. Relaxed competing-load guard retained.30s smoke `scratch_011435` with10sTTL/5sgrace passed: stored metadata and worker startup confirm5s, partial deletes1,965,752rows,4,476,500 produced/consumed once,zero application errors; DB removed. Smoke is functional verification only; study template retention_seconds120 is superseded by effective stream-settings10s for this smoke. Sustained comparison uses300–600s window, checks exact identities, maintenance errors, expired-key backlog, physical allocation and host load. Source snapshot/hash archived per study; no concurrent root changes can alter binaries.
- Bounded-grace600s pair completed (`grace_pair_20260910_011414`; zero `scratch_011513`,30s `scratch_012611`): SAME binary d84f22a8bc67b8289657d86fe50ac34e488dbab1f744cd78cd0f6b040cb2eaa6, normalized commands and all PostgreSQL settings equal, stored metadata AND startup logs verify0/30s. Current production ordered UUID key query in both; prior CTID rates not directly comparable. Common300–600s producer/consumer62,114/62,124→62,692/62,661msg/s (+0.93% observed producer, no demonstrated meaningful capacity increase). Whole-run44,396,750→39,727,500messages reflects larger baseline startup burst; every identity consumed once,zero app errors/duplicates. Consumerp993096→1284ms; max interpolated handlerbacklog242,621→40,781 with negative overall slopes in both. Six baseline maintenance failures (all key deadlines; one also partial deadline and DROP lock timeout)→ZERO with30sgrace. No exception consumer instances.
- Retention effect: partial message deletes5,340,755rows/141.262s completedSQL→ZERO;30sgrace allowed whole drops before row eligibility in this workload. Key deletes18,683,250/151.569s completedSQL→32,312,000/209.618s. Exact final retained25,713,500→7,415,500keys; expired18,407,500→ZERO at producer-stop cutoff, counted after consumers stopped. Live estimates corroborate growing baseline backlog versus small candidate oscillations; final candidate estimate421k precedes final cleanup, not contradictory to later exact0. Key allocation final3.112→1.484GB; candidate final~3minutes near1.48GB after shrink/reuse. Native peak39.55→29.91GB,hostfree minimum83.16→92.65GB,zero swapouts,100GBguard preserved. No new test/PostgreSQL containers; continuous Docker events only existing PG17 healthchecks. Ordinary Go tooling present and logged, not automatically disqualifying under agreed rules. Supports materially better retention stability around62.7k/s observed, not an absolute/indefinite maximum. Grace stays30s benchmark choice, default0.
- Monitoring limitation discovered during final analysis: BOTH sustained waits.jsonl have zero valid frames because scratch watch SQL still named pending_xmax after library rename to pending_xid. Therefore no wait-event or committed-cursor attribution from this pair; application progress/identity verification,1Hz table/storage/host sampling and final SQL counters remain valid. Original error output retained, analysis explicitly marks wait_sampling.usable=false. Corrected future runner uses pending_xid, executes query before producer launch and stops if sampler exits; syntax checked, not yet exercised against a fresh benchmark. This would have been caught by checking smoke sampler output; corrected next-run requirement includes sampler health alongside config. No repeat solely to reconstruct missing waits. All benchmark DBs removed, durable PG baseline independently verified restored. Full comparison-verdict.json and analyzer archived. Next useful load test is longer validation of30sgrace at this configuration (with repaired monitoring), before further throughput tuning.
- Post-review30minute validation started `grace_validation_20260910_020336` / `scratch_020435`, committed151c5efa clean snapshot. Current TTL+grace row cutoff;30sgrace,2m message/keyTTLs,4x250 producer,pools8/8,GOMAXPROCS4 each,GOGC400,2GiBGo limits,30s scratch cleanup timeout,10kcleanup batch/1spoll, durable nativePG18. Same100GBcap/40GiBhostfree guards; janitor active. Expected~110m messages at priorrate,200midentitybound retained. Frozen source/hash archived.30s smoke `grace_validation_5s_020357` verified313valid cursor/wait frames spanning31.1s, preventing repeat of stale pending_xmax monitoring; future runner also checks query before producer start and aborts on sampler exit or>10s stale output. Go tooling logged, new test/Postgres containers abort. Validation target: no growing message/key backlog, no maintenance failures, physical storage cycles; compare successive5min windows after startup. Owned scratch DB cleanup automatic.
- Committed30minute validation completed (`grace_validation_20260910_020336`, `scratch_020435`, commit151c5efa; binaryda3c2c9096d998f7bf59c5bc8678316f1946958a340d052fdbe8b634e46211a0):123,384,500 produced/consumed exactly once in1800.009s,zero app errors/duplicates,200midentity ceiling not hit. Excluding first5min, producer/consumer63,762/63,752msg/s; successive5min producer65,346/63,117/65,941/58,654/65,750, consumers closely matched. Consumerp991466ms; max interpolated handlerbacklog261,631 but negative overall slope; committedIDdistance median53,913, first/last30median85,581/57,152,negative slope.28completed checkpoints. No partial message DELETEs.115,936,750keydeletes in728.347s completedSQL; final retained7,447,750keys,expiredZERO at producer-stop cutoff after consumerstop. No key-cleanup timeouts. Supports logical retention/backlog stability around63.75k/s over30min, not absolute maximum.
- Physical/host result: after startup, database cycled11.36–15.46GB. Key allocation2.17GB at5min→2.98at10→3.61at15; essentiallyflat15–20min, then3.84at25→3.877at30. Reuse observed, but modest late allocation growth means indefinite physical bound unproven. Nativepeak35.10GB,hostfree minimum87.00GB,100GBtotalguard held. Late hostswapouts3460pages (~54.1MiB at16KiB/page);20–25minrate dip cannot be assigned solely toPG. No Go workload process samples and continuous Docker capture only existingPG17healthchecks, no newtestcontainers. Owned DB removed and durable baseline independently verified restored.
- Validation caveat: ONE recovered DROP lock timeout, so zero-maintenance-failure acceptance did NOT pass. PostgreSQL records message_log_4_94 timeout22:29:27.952EDT; worker warning22:29:32.340, retry1s, successfuldrop22:29:33.447. Repaired sampler produced16,716validframes, but exact wait interval has2.065s gap22:29:25.888→27.953; blocker notidentified. Existing combined sampler queries visible_head from partitioned message table before completing session observation, so DDL can delay the observer itself. Nearby WAL/cursor/extension waits do not establish this DROP's blocker. Saved exactlog, surroundingframes, lock samples,5minstorage/rates andvalidation-verdict.json. Next bounded investigation: isolate session/lock sampling from message-table reads to diagnose recurring DROP contention; retain30sgrace baseline, no blanket timeoutincrease or SSD/kernel dive. No additional load/production code changes in this turn.
- DROP contention investigation completed `drop_contention_20260910_115344` / `scratch_115405` (600s, same frozen151c5efa binary/config). Independent100ms sampler queries pg_stat_activity/pg_locks/catalog only, no message-table reads;5877validframes,118DROP-wait frames across37distinct partitions. Direct blockers include normal reader/producer transactions on parent and autovacuum on old child. Concrete07:58:38.074EDT: janitorPID97825 held AccessExclusiveLock on parent message_log_4 while requesting same mode on child message_log_4_16; autovacuumPID1186 held ShareUpdateExclusiveLock on child. Four producers97872/97958/97866/97871 and reader97839 queued on parent behindjanitor. Serverlog confirms blocking PIDs/relationOIDs; autovacuumcancelled07:58:38.210, childlockacquired201.908ms, producerparentlockacquired215.466ms. This proves a recurrent stream-wide contention chain, not the identity of the prior unsampled2s timeout blocker.
- Diagnostic confound explicitly recorded: log_lock_waits=on/deadlock_timeout200ms used to expose waits, but deadlock_timeout ALSO controls when PostgreSQL interrupts conflicting ordinary autovacuum (normally1s here). Thus not passive logging; no timeout-free/throughput gain can be attributed without matchedcontrol.48,724,000 produced/consumed once,zeroapp errors/duplicates andzerojanitorfailures; finalexpiredkeys1,013,000/retained8,826,000, so not retention-capacityvalidation. Host swapping occurred. Nativepeak35.87GB,hostfree minimum85.92GB,100GBguard held. Database removed; fsync/full_page_writes/synchronous_commitON, deadlock_timeout1000ms/log_lock_waitsOFF restored and verified. No production code changes.
- Evidence: studyverdict.json/analyzer plus run drop-blockers.json, drop-example.json, drop-example-postgres.log and bounded postgres-run.log. PostgreSQL18routine-vacuuming docs, lock-management docs and officialwiki Operations_cheat_sheet support automatic cancellation/timer caveat. Next narrowly useful test:1sversus200msdeadlock_timeout with identical independent sampler/log_lock_waitsON, keeping2sDROPtimeout anddurability unchanged. Shorter timer is a configuration candidate with extra deadlock-check/cancellation cost; no globalautovacuumdisable, no blanket cleanup timeoutincrease, no SSD/kernel investigation. If needed later, assess avoiding child-lock waiting while parent-exclusive is held; no unreviewed schema/lifecycle changes.
- 2026-09-10 matched DROP-timer pair completed: `drop_timer_pair_20260910_121045`, baseline `scratch_121106` (1000ms) and candidate `scratch_122146` (200ms), each600s, same frozen151c5efa binary SHAda3c2c9096d998f7bf59c5bc8678316f1946958a340d052fdbe8b634e46211a0, independent100ms sampler and log_lock_waitsON. Same4x250/pool8 producer, consumerpool8/claim16k/queue64k/handlers4, GOMAXPROCS4/GOGC400/GOMEMLIMIT2GiB perprocess, TTL120s/grace30s/batch10k/poll1s/cleanup30s/DROPlock2s. VS Code crashed midpair; driver/guard/load survived and both completed. Baseline host swapouts49,088pages (~804MB), candidate0; no clean causal throughput comparison. Guard had no violation; sampled gaps and tooling activity recorded in verdict.
- Timer result: autovacuum-related DROP log examples about1001ms versus201-205ms. Identical sampler observed416→96DROPwaitframes, producer session samples blocked behindDROP1502→286, consumer410→67; baseline43versus35sampled wait episodes. Autovacuum-blocked episode maximum-observed-age medians955→153ms (lower bounds, NOT exact completed wait durations;100ms sampling misses short waits). Logged autovacuum cancellations33/30. This proves a useful stall mitigation, not the main throughput ceiling; raw log wait counts have different reporting thresholds and are not compared.
- Critical monitoring correction: baseline's sole DROPtimeout was benchmark-induced. Backend14507 ran the runner's legacy1Hz `SELECT max(id) FROM sqlstreams.message_log_4` snapshot; it blocked DROPpid8352 of partition30 during08:18:33-35, timing out08:18:35.258, with a producer held~1.99s. Application warning arrived08:18:46.747, so warning-time-only investigation would misattribute the blocker.20direct frames archived in scratch_121106/timeout-blockers.json. Independent100ms sampler avoided table reads, but older main-loop snapshot still read the parent. AFTER completion removed MAX(id)/MAX(committed) from temporary drop-runner, retaining activity snapshot; archived `runner-next-no-message-poll.py`, Python compile verified, not yet load-tested. Measured original preserved as runner.py. Do not classify this timeout as a production janitor defect.
- Application/retention results:48,116,000 versus42,264,250 messages produced/consumed exactly once, zeroapp errors/duplicates.300-600s producer/consumer61,915/61,945 versus68,132/68,159msg/s; cannot attribute~10%late uplift given host disturbance (whole-run80,186versus70,440msg/s reverses ranking). Consumerp991271versus1871ms; interpolated maxhandlerbacklog42,319versus349,389, overall slopesnegative. Janitorfailures1monitor-inducedversus0. Finalexpiredkeys0versus21,500; retained7,443,250versus8,598,250. No new sustainable maximum or general latency improvement claimed.
- Both scratchDBs removed; nativepeak34.58/30.06GB, minimumhostfree87.06/101.45GB,100GBtotal guard held. Baseline restored and directly verified: deadlock_timeout1000ms, log_lock_waitsOFF, fsync/synchronous_commit/full_page_writesON. No library changes or commits. Pairverdict/analyzer, per-arm comparisons, lockepisodes/blockers, bounded PostgreSQLlogs and source/config archives retained. Next: use corrected non-table-reading monitor for future validation;200ms remains benchmark candidate, no production-default change or larger DROP redesign justified by this pair.
- 2026-09-10 corrected-monitor30minute validation completed: `drop_clean_validation_20260910_124746`, run `scratch_124808`, phase study `drop_timer_200ms_124808`.1800.042s with same frozen151c5efa binary/config (4x250producer/pool8; consumerpool8/claim16k/queue64k/handlers4; GOMAXPROCS4/GOGC400/GOMEMLIMIT2GiB each; nativePG18.6/shared_buffers6GB; TTL120s/grace30s/partition1m/batch10k/janitorpoll1s/cleanup30s/DROPlock2s), deadlock_timeout200ms/log_lock_waitsON. Archived runner confirms old parentMAX(id)/cursor polling removed; independent100ms lock sampler and activity-only main snapshot retained. No library changes.
- Message throughput validated for this30minute window:129,929,250produced/consumed exactly once; zeroapp errors/duplicates andzerojanitor failures.300-1800s producer65,677.66/consumer65,673.95msg/s. Five-minute producer windows afterstartup68,324 /64,976 /63,457 /64,005 /67,627. Late interpolatedhandlerbacklog slope-0.705msg/s, first60mean13,949→last60mean12,270; transientmax237,772. Consumerp99upperbound1500ms. No growing consumerbacklog, no claim of absolute maximum or causal timerthroughputuplift.
- Cleanup/storage qualification:121,377,000key deletes by final1Hzcounter; directfinal8,319,000retainedkeys with118,500expired at producerstopcutoff. Estimatedexpirybacklog oscillated/recovered (lastminute~410k→376.5k by delayedstats; directfinalcount authoritative). KeyallocatedGB at10/15/20/25/30min4.168/5.287/6.652/6.814/7.704; plateau ended and growth resumed. Finalestimateddeadrows71.22m; completedkeyautovacuums3at10min,4at30min. Livekeys stayedroughly8-9m late. Therefore applicationthroughput/cleanupfailure validation passes, but indefinitelystable storage NOT established; deleted-row reclamation/retained allocation warrants a bounded check before adoption as fullsteady-state storagebaseline. Do not infer partitioning requirement from this alone.
- Lock/host evidence:17,457validlockframes,maxgap1.665s;399DROPwaitframes/143sampledwaitepisodes; noDROPtimeout.104loggedautovacuumcancellations; someclient-blockedDROPwaits remain (maxobservedage1.799s, lowerbound). Host0swapoutpages/225swapinpages, compressionactivity remains;32checkpoints. Nativepeak35.761GB,minhostfree95.399GB,100GBtotalstorageguard held. Guardcompleted without violation, maxgap2.539s;14ordinaryGo vet/testmatchedframes (~14s) recorded under relaxedrules, not autoaborted; no newtestPGcontainer. Hostnot assumedexclusive.
- ScratchDB removed and baseline directlyverified: deadlock_timeout1000ms/log_lock_waitsOFF,fsync/synchronous_commit/full_page_writesON. Source/config, effectivejanitor/runtimeverification, rawprofiles/locks, boundedPGlog, per-arm comparison, key-backlog-trend and studyverdict archived. Nextnarrowstep: inspect recorded idempotency vacuum progress/waits/settings and consider one per-table autovacuum configuration comparison; do not reopen SSD/kernel attribution, disableautovacuum/durability, or redesigntables yet.200ms remains a benchmarkconfigurationcandidate; no productiondefaultchange, no commit.
- 2026-09-10 user-directed smaller-delete/manual-VACUUM comparison completed. Prior small-batch evidence used older pre-grace cleanup and was not reused as a matched control. Study `delete_batches_20260910_142715`: three600s runs with10k/5k/1k cleanup batches; follow-up `manual_key_vacuum_20260910_150033`:600s at10k plus explicit VACUUM(ANALYZE,VERBOSE) on idempotency_key_4 at180/300/420/540s. Same frozen151c5efa binary acrossall4runs,4x250producer/pool8,consumerpool8/claim16k/queue64k/handlers4,GOMAXPROCS4/GOGC400/GOMEMLIMIT2GiB each,TTL120/grace30/poll1/cleanup30/DROPlock2/deadlock_timeout200ms/log_lock_waitsON. Runtime batchmetadata verified. Catalog-only monitor additionally records vacuum progress and separate keyheap/index sizes; no intrusive MAX(id) probe. No partial orders message DELETEs in any arm.
- Batchresults10k=`scratch_142736`:50,284,250messagesonce,late300-600produce/consume65,374/65,340msg/s,0maintenancefailures,458,250expiredkeys atfinalcheck; successfulDELETEmean61.81ms/~9694rows,110.28WALbytes/deletedrow.5k=`scratch_143816`:39,932,500once,62,048/62,045msg/s,0maintenancefailures,598,000expired;35.99ms/~4888rows,109.80WALbytes/row.1k=`scratch_144859`:39,756,500once,62,070/62,090msg/s,6keycleanupdeadlineevents (4warninglines+2suppressed),14,821,000expired;successfulDELETE8.66ms/~993rows,114.20WALbytes/row. Canceledstatementcost omitted fromsuccessfulSQLstats, so short average1k transaction does NOT imply efficient totalcleanup. Smaller transactions shortened individual calls but did not cut WAL/row or improve throughput;1k failed cleanup. Retained10kforvacuumtest, no productiondefaultchange. Single sequentialscreens/unequalinitialbursts do not prove causal5%throughputdifference.
