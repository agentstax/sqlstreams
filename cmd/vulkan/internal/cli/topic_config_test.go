package cli

import (
	"testing"
	"time"

	"github.com/agentstax/vulkan/pkg/topic"
)

func TestTopicConfigIncludesEmptyCompactionHeadTTL(t *testing.T) {
	entry, ok := findTopicConfigKey("empty_compaction_head_ttl")
	if !ok {
		t.Fatal("empty_compaction_head_ttl config key is missing")
	}
	defaults := (&topic.TopicConfig{}).WithDefaults().ToTopic(0, 0, "")
	if got := entry.read(defaults); got != "1h0m0s" {
		t.Fatalf("default output = %q, want %q", got, "1h0m0s")
	}
}

func TestTopicDocumentIncludesEmptyCompactionHeadTTL(t *testing.T) {
	document := toTopicDocument(&topic.Topic{EmptyCompactionHeadTTL: 2 * time.Hour})
	if document.EmptyCompactionHeadTTL != "2h0m0s" {
		t.Fatalf("EmptyCompactionHeadTTL = %q, want %q", document.EmptyCompactionHeadTTL, "2h0m0s")
	}
}
