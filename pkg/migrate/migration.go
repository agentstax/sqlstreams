...
// a streamId that the system scope ignores (always 0) and the stream scope uses
// for per-stream tables. Both are spelled in the SQL --
// fmt.Sprintf("... %s.message_log_%d ...", schema, streamId) -- because the
...
type Migration struct {
	...
	ValidateUp   func(ctx context.Context, q datastore.Querier, schema string, streamId int64) error // preconditions; nil = none
	Up           func(ctx context.Context, q datastore.Querier, schema string, streamId int64) error // idempotent -- a retry may re-run it
	ValidateDown func(ctx context.Context, q datastore.Querier, schema string, streamId int64) error
	Down         func(ctx context.Context, q datastore.Querier, schema string, streamId int64) error
