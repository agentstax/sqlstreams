package datastore

import (
	"context"
	"fmt"

	"github.com/agentstax/sqlstreams/pkg/stream"
)

// SweepExpiredKeyLeases deletes this stream's expired message_key_lease rows.
// A crashed consumer leaves its expired row behind:
//   - the key gets another message -> that claim takes the row over
//   - the key never does -> the row sits forever, only this sweep removes it
func (d *JanitorDatastore) SweepExpiredKeyLeases(ctx context.Context, streamId int64, batchSize int) error {
	return d.DatastoreRetry.Wrap(ctx, func() error {
		return d.sweepExpiredKeyLeases(ctx, streamId, batchSize)
	})
}

func (d *JanitorDatastore) sweepExpiredKeyLeases(ctx context.Context, streamId int64, batchSize int) error {
	// protect against any potential infinite loops
	const maxKeyLeaseSweepBatches = 1000
	for range maxKeyLeaseSweepBatches {
		swept, err := d.sweepKeyLeasesBatch(ctx, streamId, batchSize)
		if err != nil {
			return err
		}
		if swept < batchSize {
			break // ran out of expired rows
		}
	}
	return nil
}

// racing a consumer acquiring the same key is safe: if the delete wins, the
// consumer's upsert inserts a fresh row instead of updating the expired one
func (d *JanitorDatastore) sweepKeyLeasesBatch(ctx context.Context, streamId int64, batchSize int) (int, error) {
	sql := fmt.Sprintf(`
		-- sqlstreams: streamjanitor.sweepKeyLeasesBatch
		DELETE FROM %[1]s.%[2]s
		WHERE (consumer_group_id, message_key) IN (
			SELECT k.consumer_group_id, k.message_key
			FROM %[1]s.%[2]s k
			WHERE k.expires_at < now()
			LIMIT $1
		);
	`, d.Datastore.Schema, stream.MessageKeyLeaseTable(streamId))
	tag, err := d.Datastore.Pool.Exec(ctx, sql, batchSize)
	if err != nil {
		return 0, err
	}

	return int(tag.RowsAffected()), nil
}
