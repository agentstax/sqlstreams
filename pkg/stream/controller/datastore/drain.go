...
//
// So a stream's log can't be destroyed with one DROP of the partitioned
...
func (d *StreamDatastore) drainPartitions(ctx context.Context, streamId int64) error {
	parentTableName := fmt.Sprintf("%s.%s", d.Datastore.Schema, stream.MessageLogTable(streamId))

	countSql := `
		-- sqlstreams: stream.drainPartitions
	...
	// went past passLimit
	return stream.ErrStreamPartitionsRemain.With("stream_id", streamId)
}

func (d *StreamDatastore) listPartitions(ctx context.Context, parentTableName string) ([]string, error) {
	// to_regclass, not ::regclass -- a missing parent yields NULL, not error
	sql := `
		-- sqlstreams: stream.listPartitions
		SELECT c.relname
		FROM pg_inherits i
...
func (d *StreamDatastore) dropPartitionBatch(ctx context.Context, partitions []string) error {
	...
	for _, partition := range partitions {
		...
		dropSql := fmt.Sprintf(`
			-- sqlstreams: stream.dropPartitionBatch
