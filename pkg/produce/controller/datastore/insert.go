...
// already-open tx.
func (d *ProduceDatastore) runInsert[Message common.Versioned](ctx context.Context, tx iDatastore.Tx, streamId int64, produceFunc produce.ProducerFunc[Message], data *Append[Message]) (*Appended[Message], error) {
	...
	id, duplicate, err := d.insertProtected(ctx, tx, streamId, payload, data)
	if err != nil {
		return nil, err
	}
...
func (d *ProduceDatastore) runInsertSavepoint[Message common.Versioned](ctx context.Context, tx iDatastore.Tx, streamId int64, produceFunc produce.ProducerFunc[Message], data *Append[Message]) (*Appended[Message], error) {
	...
	id, duplicate, err := d.insertProtectedSavepoint(ctx, tx, streamId, payload, data)
	if err != nil {
		attemptRollbackToSavepoint(ctx, tx, produceInTxSavepoint)
		return nil, err
	}
...
// already existed.
func (d *ProduceDatastore) insertProtectedSavepoint[Message common.Versioned](ctx context.Context, q iDatastore.Querier, streamId int64, payload *Message, data *Append[Message]) (id int64, duplicate bool, err error) {
	sql, args, err := protectedInsertSQL(streamId, payload, data, d.Datastore.Schema)
	if err != nil {
		return 0, false, err
	}
...
func (d *ProduceDatastore) insertProtected[Message common.Versioned](ctx context.Context, q iDatastore.Querier, streamId int64, payload *Message, data *Append[Message]) (id int64, duplicate bool, err error) {
	sql, args, err := protectedInsertSQL(streamId, payload, data, d.Datastore.Schema)
	if err != nil {
		return 0, false, err
	}
...
// exact same statement. Claims against idempotency_key_<streamId>.
//
...
func protectedInsertSQL[Message common.Versioned](streamId int64, payload *Message, data *Append[Message], schema string) (string, []any, error) {
	...
	if data.Compacted {
		...
		`, schema, stream.IdempotencyKeyTable(streamId), stream.MessageLogTable(streamId), stream.CompactionHeadTable(streamId))
		...
		sql = fmt.Sprintf(`
			...
			WHERE EXISTS (SELECT 1 FROM claim) -- if claim CTE didn't return anything skip this
			RETURNING id;
		`, schema, stream.IdempotencyKeyTable(streamId), stream.MessageLogTable(streamId))
