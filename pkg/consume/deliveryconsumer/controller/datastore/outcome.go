...
func (d *DeliveryConsumerGroupDatastore) recordTerminal(ctx context.Context, delivery *ExceptionQueueRow, terminalErr error, deliveryLogMode topic.DeliveryLogMode) error {
	...
	d.Logger.WarnContext(ctx, consume.EventMessageDeadLettered.Message(), "code", consume.EventMessageDeadLettered.GetCode(), "group_id", delivery.ConsumerGroupId, "topic_id", delivery.TopicId, "message_id", delivery.MessageId)
	return nil
}
