...
func (i *CompactionReadCostInstance) evaluateStreams(ctx context.Context, jobPayload *alert.JobPayload) error {
	...
	for _, listed := range streams {
		...
		outcome, err := i.alerts.Record(ctx, alert.AlertCompactionReadCost.Name, owner, result)
		if err != nil {
			failed++
			errs = errors.Join(errs, err)
			continue
