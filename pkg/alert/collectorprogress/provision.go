...
func (p *CollectorProgressProvisioner) Declare(ctx context.Context, owner *common.Owner) error {
...
func (p *CollectorProgressProvisioner) Provision(ctx context.Context, declared *worker.Worker) (worker.Execution, error) {
	parsed, err := workercontroller.ParseMetadata[collectorProgressMetadata](declared.Metadata)
	if err != nil {
		return nil, err
	}
	...
	return newCollectorProgressInstance(p, declared.Owner, claimed, parsed.RepeatInterval)
}
