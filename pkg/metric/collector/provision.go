...
func (d *MetricCollectorProvisioner) Declare(ctx context.Context, owner *common.Owner) error {
...
func (d *MetricCollectorProvisioner) Provision(ctx context.Context, declared *worker.Worker) (worker.Execution, error) {
	parsed, err := controller.ParseMetadata[metricCollectorMetadata](declared.Metadata)
	if err != nil {
		return nil, err
	}
	...
	return newMetricCollectorInstance(d, declared.Owner, claimed, parsed, producerInstance)
}
