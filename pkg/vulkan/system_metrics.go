...
func (s *SystemMetricsHandle) Consumer(name string) *ConsumerHandle[Measurement] {
	return s.client.Topic[Measurement](metric.MetricTopicName).Consumer(name)
}
...
func (s *SystemMetricsHandle) CheckTopicsEvaluated(alert string) *MetricHandle {
	...
	return s.metric(metric.MetricCheckTopicsEvaluated, attributes)
}
...
func (s *SystemMetricsHandle) CheckTopicsFailed(alert string) *MetricHandle {
	attributes := map[string]string{"alert": alert}
	return s.metric(metric.MetricCheckTopicsFailed, attributes)
}
