...
func TestOwnerColumnsSelectsOnlyOwningResource(t *testing.T) {
	for _, sample := range []struct {
		...
		{"stream", common.Owner{SystemId: 1, StreamId: 12}, [3]int64{0, 12, 0}},
		{"consumer group", common.Owner{SystemId: 1, StreamId: 12, ConsumerGroupId: 34}, [3]int64{0, 0, 34}},
	} {
		t.Run(sample.name, func(t *testing.T) {
			columns := NewOwnerColumns(sample.owner)
			for index, value := range []*int64{columns.SystemId, columns.StreamId, columns.ConsumerGroupId} {
