...
func (d *JanitorDatastore) SweepExpiredEmptyCompactionHeads(ctx context.Context, topicId int64, ttl time.Duration, batchSize int) error {
	return d.DatastoreRetry.Wrap(ctx, func() error {
		return d.sweepExpiredEmptyCompactionHeads(ctx, topicId, ttl, batchSize)
	})
}
...
func (d *JanitorDatastore) sweepExpiredEmptyCompactionHeads(ctx context.Context, topicId int64, ttl time.Duration, batchSize int) error {
	...
	for range maxEmptyCompactionHeadSweepBatches {
		swept, err := d.sweepEmptyCompactionHeadsBatch(ctx, topicId, cutoff, batchSize)
		if err != nil {
			return err
		}
		if swept < batchSize {
...
func (d *JanitorDatastore) sweepEmptyCompactionHeadsBatch(ctx context.Context, topicId int64, cutoff time.Time, batchSize int) (int, error) {
	sql := fmt.Sprintf(`
		-- vulkan: topicjanitor.sweepEmptyCompactionHeadsBatch
		WITH expired AS (
	...
	if swept > 0 {
		d.Logger.DebugContext(ctx, "swept expired empty compaction heads", "topic_id", topicId, "swept_count", swept, "batch_size", batchSize)
	}
	return swept, nil
}
