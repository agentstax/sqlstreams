	...
	for _, cancelParent := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			controller, err := janitorcontroller.NewJanitorController(&datastore.PostgresDatastore{
				Pool: pool, Schema: "vulkan", Logger: logger, Retry: common.NewDefaultRetryPolicy(),
			}, logger)
			if err != nil {
				t.Fatal(err)
			}
			instance := &JanitorInstance{
				Topic:      &topic.Topic{Id: 42, PartitionSize: 1000000, RetentionTTL: time.Hour, IdempotencyKeyTTL: time.Hour, EmptyCompactionHeadTTL: time.Hour, DeliveryLogMode: topic.DeliveryLogModeOff},
