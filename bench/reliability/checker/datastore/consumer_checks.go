...
func (d *CheckerDatastore) CountReclaims(ctx context.Context, target Target) (Measurement, error) {
	reclaimsSql := fmt.Sprintf(`
		-- lab: datastore.CountReclaims
		SELECT
			count(*),
			COALESCE((array_agg(message_id::text ORDER BY message_id))[1:%[2]d], ARRAY[]::text[])
		FROM %[1]s
		WHERE consumer_group_id = $1 AND status = 'expired';
	`, target.deliveryLog(), exampleLimit)
	return d.measure(ctx, exampleMessageId, reclaimsSql, target.GroupId)
}

// Dead: exception rows the group dead-lettered.
func (d *CheckerDatastore) CountDead(ctx context.Context, target Target) (Measurement, error) {
	deadSql := fmt.Sprintf(`
		-- lab: datastore.CountDead
		SELECT
			count(*),
			COALESCE((array_agg(message_id::text ORDER BY message_id))[1:%[2]d], ARRAY[]::text[])
		FROM %[1]s
		WHERE consumer_group_id = $1 AND status = 'dead';
	`, target.exceptionQueue(), exampleLimit)
	return d.measure(ctx, exampleMessageId, deadSql, target.GroupId)
}
