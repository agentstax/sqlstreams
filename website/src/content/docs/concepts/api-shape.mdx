...
`github.com/agentstax/vulkan/pkg/vulkan`; the per-handle lookup tables are
the [Reference board](/boards/reference/).

## Two grammars

The client selects the noun: `Topic[T](name)`, `Scheduler(name)`,
`System()`, `Manager()`. A topic handle selects its own: `Consumer(name)`,
`Producer()`, `Key(messageKey)`. A handle uses the bare verb,
`orders.Register`, `orders.Destroy`, `nightly.Suspend`: it already knows
what it is, and `orders.DestroyTopic` says topic twice. A plural method
reads the materialized collection: `Topics`, `Schedulers`, `Consumers`.
...
The topic carries the message type and everything under it inherits it.
`client.Topic[OrderPlaced]("orders")` is spelled once, and the group's
...
handler's parameter type is the topic's.

The admin verbs never look at it. `Register`, `Destroy`, `Migrate`,
`Rename`, `Health`, and `Metrics` run the same for any type, so a topic
holding two schema versions is two handles on the same rows:
`Topic[OrderPlacedV1]` reads V1 payloads, `Topic[OrderPlacedV2]` reads V2,
and either one destroys the topic.

A caller that does not know the type, the CLI and any admin-only script,
uses `vulkan.RawPayload`: the stored JSON bytes as they are, declaring
...
Newest wins, every time. `Register` on a topic, a consumer group, a
...
([VK0061](/errors/VK0061/) for a topic, [VK0059](/errors/VK0059/) for a
group, [VK0062](/errors/VK0062/) for a schedule), and an identical one
...
tell is the same VK0059 line on every restart of either one. One declaring
service per resource is a convention, not a constraint. A refusing form was
built and reverted: a service that refuses a differing config can never
have its config changed without first redeploying it as accepting.
...
client (`TopicHandle`, `ConsumerHandle`, `KeyHandle`, `SchedulerHandle`);
the bare noun is the materialized value returned by `Get` (`Topic`,
...
`ScheduleConsumerGroupSummary`, `TopicVersionHealth`. `Data` and `Info` are
...
stability commitment; `vulkan.RetryPolicy` aliases a type declared in
...
`vulkan` is a large package: every config, options, error, and read-model
...
the small interface it consumes beside its own code; Vulkan does not ship
