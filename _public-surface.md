...
| Keep | `RetryPolicy.CalculateDelay`, `CalculateTotalDelay`, `Equal` | Contracts documented and tested. Validate now rejects calculated total retry sleep overflow; overflow checks live directly in Validate, while CalculateTotalDelay only calculates the validated budget; both handle constant and capped tails without iterating every sleep. Oversized budgets previously accepted now fail validation. Producer construction rejects combined sleep/operation-budget overflow before multiplication or addition. CalculateDelay now compares backoff with the cap before conversion and returns the original duration cap; below-cap floating-point calculation remains unchanged. NewDefaultRetryPolicy and RetryableFunc remain advanced-only. |
...
| `RetryPolicy` | [common.RetryPolicy](pkg/common/retry_policy.go) | `CalculateDelay`, `CalculateTotalDelay`, `Equal`, `WithDefaults`, `Validate` | Keep |
