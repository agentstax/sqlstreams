...
| Question | `Client.Datastore()`, `PostgresDatastore` alias | Recommend keep: CLI manager passes the existing datastore to otelvulkan.NewExporter; advanced controllers and labs share it too. Proposed contract in the client guide: fields are construction-time state, readable and usable by integrations; replacing Pool/Schema/Logger/Retry or mutating shared Retry after construction is unsupported. Components capture some values, so this is not live reconfiguration. Caller retains pool ownership. No wrapper or copied datastore proposed; review Client.Config/Logger separately. |
...
| [system_metrics.go](pkg/vulkan/system_metrics.go) | `SystemMetricsHandle`, `SystemHandle.Metrics`, `SystemMetricsHandle.Definitions`, `SystemMetricsHandle.Latest`, `SystemMetricsHandle.Metric`, `SystemMetricsHandle.UnclaimedWorkers`, `SystemMetricsHandle.OldestUnclaimedAge`, `SystemMetricsHandle.FailingWorkers`, `SystemMetricsHandle.OverdueSchedules`, `SystemMetricsHandle.OldestDueAge`, `SystemMetricsHandle.SuspendedSchedules`, `SystemMetricsHandle.ActiveAlerts`, `SystemMetricsHandle.ResolvedAlerts`, `SystemMetricsHandle.CheckTopicsEvaluated`, `SystemMetricsHandle.CheckTopicsFailed`, `SystemMetricsHandle.CheckPublishedAlerts`, `SystemMetricsHandle.CheckResolvedAlerts` | Keep |
