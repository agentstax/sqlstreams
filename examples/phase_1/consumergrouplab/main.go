...
func run() (err error) {
	...
	headInstance, err := client.Topic[labMessage](topicA.Name).Consumer(headGroup).Register(ctx, &vulkan.ConsumerConfig{
		Start: vulkan.Head(),
	})
	...
	_, err = client.Topic[labMessage](topicA.Name).Consumer(headGroup).Register(ctx, nil)
	must(err)
