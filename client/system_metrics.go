...
// Consumer names a consumer on the system metrics stream. No I/O.
...
func (s *SystemMetricsHandle) SuspendedSchedules() *MetricHandle {
	return s.metric(metric.MetricSuspendedSchedules, nil)
}

// ActiveAlerts selects the system's active-alert series.
func (s *SystemMetricsHandle) ActiveAlerts() *MetricHandle {
	return s.metric(metric.MetricActiveAlerts, nil)
}

// ResolvedAlerts selects the system's resolved-alert series.
func (s *SystemMetricsHandle) ResolvedAlerts() *MetricHandle {
	return s.metric(metric.MetricResolvedAlerts, nil)
}

// CheckStreamsEvaluated selects the streams-evaluated series for one alert
// check.
func (s *SystemMetricsHandle) CheckStreamsEvaluated(alert string) *MetricHandle {
	attributes := map[string]string{"alert": alert}
	return s.metric(metric.MetricCheckStreamsEvaluated, attributes)
}

// CheckStreamsFailed selects the streams-failed series for one alert check.
func (s *SystemMetricsHandle) CheckStreamsFailed(alert string) *MetricHandle {
	attributes := map[string]string{"alert": alert}
	return s.metric(metric.MetricCheckStreamsFailed, attributes)
}
...
func (s *SystemMetricsHandle) CheckPublishedAlerts(alert string) *MetricHandle {
	attributes := map[string]string{"alert": alert}
	return s.metric(metric.MetricCheckPublishedAlerts, attributes)
}
...
func (s *SystemMetricsHandle) CheckResolvedAlerts(alert string) *MetricHandle {
	attributes := map[string]string{"alert": alert}
	return s.metric(metric.MetricCheckResolvedAlerts, attributes)
}
