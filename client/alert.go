...
type AlertHandle struct {
	name       string
	streamName string // "" for a system-owned alert
	groupName  string // "" unless the owner is a consumer group
	client     *Client
}

func newAlertHandle(client *Client, name string, streamName string, groupName string) *AlertHandle {
	return &AlertHandle{name: name, streamName: streamName, groupName: groupName, client: client}
}
...
// Returns ErrStreamNotFound or ErrConsumerNotFound for a missing owner and
...
func (a *AlertHandle) owner(ctx context.Context) (*common.Owner, error) {
	switch {
	case a.groupName != "":
		return a.client.admin.ConsumerGroupOwner(ctx, a.streamName, a.groupName)
	case a.streamName != "":
		return a.client.admin.StreamOwner(ctx, a.streamName)
	default:
