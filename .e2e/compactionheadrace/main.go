...
// compaction_head correctness e2e test: concurrent-write convergence, plus the O(1)
// counterpart to compactionscale's linear growth curve.
//
// Part 1 -- concurrent same-key race. Every other e2e test in this phase only
...
// same checkpoints as compactionscale -- this e2e test's whole point is a
...
func run() (err error) {
	...
	fmt.Println("\n✅ LATEST KEYS RACE + SCALE E2E TEST PASSED")
	return nil
}
...
// scaleCurveScenario: identical seeding shape to compactionscale, but
...
func scaleCurveScenario(ctx context.Context, pool *pgxpool.Pool) {
	...
	fmt.Println("  -> unlike compactionscale's old NOT EXISTS scan (touched grew with history size,")
