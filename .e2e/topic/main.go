...
func run() (err error) {
	...
	topicA := register(fmt.Sprintf("phase8b.topic.a.%d", run))
	topicB := register(fmt.Sprintf("phase8b.topic.b.%d", run))
	topicC := register(fmt.Sprintf("phase8b.topic.c.%d", run))
	topicD := register(fmt.Sprintf("phase8b.topic.d.%d", run))
	defer func() {
	...
	groupA := "topic.groupA" // topicA's own reader, fully caught up
	groupAID := mustGroupID(cd.RegisterGroup(ctx, topicA.Id, groupA, consume.Beginning()))
	setCursor(ctx, ds, topicA.Id, groupAID, 5, 5)

	groupB := "topic.groupB" // topicB's reader, registered but never advances -- badly lagging
	...
	// ===== PROOF 3: routing_key/bindings still behave as Phase 7/routing proved, now scoped to one topic =====
	step("PROOF 3: routing_key/bindings behave as Phase 7 proved, scoped within one topic (condensed -- full suite in routing)")
	wpCInstance, err := client.Topic[common.Work](topicC.Name).Producer().Register(ctx, nil)
	must(err)
	groupRoute := "topic.route"
	...
	must(err)
	groupX := "topic.sliceX" // reads only sliceX.* -- will be fully caught up
	groupY := "topic.sliceY" // reads only sliceY.* -- registered but stays lagging
