...
// Phase 8a e2e test (a): id-range partitioning prunes claim reads to 1-2 partitions.
//
...
// discarding whatever rows were in it). A dedicated stream gets its own
// message_log_<id> at exactly the width this e2e test wants, and Destroy cleans it
...
func run() (err error) {
	...
	for i := int64(1); i <= 14; i++ {
		...
		// heal-and-burn an id if it won) -- waiting here keeps the e2e test's id
	...
	fmt.Println("\n✅ PARTITION PRUNING E2E TEST PASSED")
...
func explainReadMessages(ctx context.Context, ds *iDatastore.PostgresDatastore, streamId, low, high int64, want int) {
	logName := stream.MessageLogTable(streamId)
	logTable := fmt.Sprintf("%s.%s", ds.Schema, logName)
	bindingTable := fmt.Sprintf("%s.%s", ds.Schema, stream.BindingConfigTable(streamId))
	sql := fmt.Sprintf(`
